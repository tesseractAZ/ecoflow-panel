...
No real emergency is muted (adversarially verified: the only thing suppressible is a >25-pt single-tick collapse that is simultaneously incoherent and from a fresh healthy baseline — physically impossible for this pool). +11 tests (6 coherence: real/incident/null-together/missing-member/coherent-zero/boundary; 5 engine: transient-0 no-cascade / sustained-0 / gradual-still-fires / deep-discharge-still-fires / stale-baseline-re-baselines); suite 776 → 787; `tsc` clean. Tunable via `BATTERY_SOC_MAX_DROP_PCT`.
