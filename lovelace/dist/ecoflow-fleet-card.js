...
`;function xe(e){return{critical:e.filter(e=>"critical"===e.severity).length,warning:e.filter(e=>"warning"===e.severity).length,info:e.filter(e=>"info"===e.severity).length}}function ke(e){const t=e.match(/(\d+)\s*$/);return t?Number(t[1]):null}function _e(e,t){const s=e=>{const t=(e.productName??"").toLowerCase();return t.includes("smart home panel")?0:t.includes("delta pro ultra")?1:t.includes("delta 3 plus")?2:3},r=s(e),a=s(t);if(r!==a)return r-a;if(1===r){const s=ke(e.deviceName),r=ke(t.deviceName);if(null!=s&&null!=r)return s-r;if(null!=s)return-1;if(null!=r)return 1}return e.deviceName.localeCompare(t.deviceName)}const Se=e=>null==e?"—":Math.abs(e)>=1e3?`${(e/1e3).toFixed(2)} kW`:`${Math.round(e)} W`,Ae=(e,t=0)=>null==e?"—":`${e.toFixed(t)}%`,Pe=e=>null==e?"—":`${Math.round((e=>9*e/5+32)(e))}°F`,Ce=e=>{if(null==e)return"—";if(e<60)return`${Math.round(e)} min`;const t=Math.floor(e/60),s=Math.round(e%60);if(t<24)return`${t}h ${s}m`;return`${Math.floor(t/24)}d ${t%24}h`},Ee={};function Te(e,t){for(const s of e.split("|"))Ee[s.trim()]=t}function Me(e){const t=function(e){const t=function(e){return e.split("·")[0].split("(")[0].replace(/\s+/g," ").trim().toLowerCase()}(e);return t?Ee[t]:void 0}(e);return t?V`<span class="ef-glossary"
    >${e}<span class="ef-tooltip" role="tooltip">${t}</span></span
  >`:e}function He(e,t,s,r){const a=t-e||1,i=r-s;return t=>s+(t-e)/a*i}function je(e,t,s){const r=[];let a=!1;for(const i of e){if(null==i.value||!Number.isFinite(i.value)){a=!1;continue}const e=t(i.ts),o=s(i.value);r.push(`${a?"L":"M"} ${e.toFixed(1)} ${o.toFixed(1)}`),a=!0}return r.join(" ")}function We(e,t={}){const s=t.width??720,r=t.height??220,a=36,i=10,o=s-a-36,n=r-i-22,l=[...e.area?.points??[],...e.line?.points??[],...e.rightLine?.points??[]];if(l.length<2)return V`<div style="height:${r}px;color:var(--ef-muted);font-size:11px;display:flex;align-items:center;justify-content:center;">no forecast data</div>`;const c=l.map(e=>e.ts),d=Math.min(...c),h=Math.max(...c),p=[];e.area&&p.push(...e.area.points.map(e=>e.value).filter(e=>null!=e)),e.line&&p.push(...e.line.points.map(e=>e.value).filter(e=>null!=e));const u=t.yMax??1.05*Math.max(100,...p),f=Math.min(0,...p),v=He(d,h,a,a+o),g=He(f,u,i+n,i),m=He(0,100,i+n,i),$=g(0),b=216e5,y=[];for(let e=Math.ceil(d/b)*b;e<=h;e+=b){const t=v(e);y.push(q`<line x1=${t} x2=${t} y1=${i} y2=${i+n} stroke="var(--ef-line)" stroke-dasharray="2 3" stroke-opacity=".6" />`)}const w=[0,u/2,u].map(e=>({v:e,y:g(e)})),x=[0,50,100].map(e=>({v:e,y:m(e)})),k=e.area?function(e,t,s,r){const a=je(e,t,s);if(!a)return"";let i=null,o=null;for(const s of e)if(null!=s.value&&Number.isFinite(s.value)){const e=t(s.ts);null==i&&(i=e),o=e}return null==i||null==o?"":`${a} L ${o.toFixed(1)} ${r.toFixed(1)} L ${i.toFixed(1)} ${r.toFixed(1)} Z`}(e.area.points,v,g,$):"",_=e.line?je(e.line.points,v,g):"",S=e.rightLine?je(e.rightLine.points,v,m):"",A=e.rightRef?m(e.rightRef.value):null,P="display:inline-block;width:14px;height:2px;margin-right:4px;vertical-align:middle",C=V`<div style="display:flex;flex-wrap:wrap;gap:10px;font-size:11px;color:var(--ef-muted);margin-top:4px;">${e.area?.label?V`<span><span style="${"display:inline-block;width:10px;height:10px;opacity:.6;border-radius:2px;margin-right:4px"};background:${e.area.color};"></span>${e.area.label}</span>`:null}${e.line?.label?V`<span><span style="${P};background:${e.line.color};"></span>${e.line.label}</span>`:null}${e.rightLine?.label?V`<span><span style="${P};background:${e.rightLine.color};"></span>${e.rightLine.label}</span>`:null}</div>`;return V`<svg viewBox="0 0 ${s} ${r}" width="100%" height=${r} preserveAspectRatio="none" aria-hidden="true">${y}${w.map(e=>q`<line x1=${a} x2=${a+o} y1=${e.y} y2=${e.y} stroke="var(--ef-line)" stroke-opacity=".4" /><text x=${32} y=${e.y+3} text-anchor="end" font-size="9" fill="var(--ef-muted)">${(e.v/1e3).toFixed(1)}k</text>`)}${x.map(e=>q`<text x=${a+o+4} y=${e.y+3} text-anchor="start" font-size="9" fill="var(--ef-muted)">${e.v.toFixed(0)}%</text>`)}${null!=A?q`<line x1=${a} x2=${a+o} y1=${A} y2=${A} stroke=${e.rightRef.color} stroke-dasharray="4 4" stroke-opacity=".7" />`:null}${k?q`<path d=${k} fill=${e.area.color} fill-opacity=".35" stroke="none" />`:null}${_?q`<path d=${_} fill="none" stroke=${e.line.color} stroke-width="1.6" />`:null}${S?q`<path d=${S} fill="none" stroke=${e.rightLine.color} stroke-width="2" />`:null}</svg>${C}`}Te("soc|state of charge","State of charge — how full the battery is right now, 0–100%."),Te("avg soc","Average state of charge across every online battery pack in the fleet."),Te("soh|state of health|avg soh","State of health — measured usable capacity vs the pack’s original design capacity. A wear gauge; 100% = like-new."),Te("ocv|open-circuit","Open-circuit voltage — the pack’s resting voltage with no load applied."),Te("cell spread|worst cell spread|cell imbalance|cell spread now","Cell-voltage spread — the gap between the highest and lowest cell in a pack. A widening gap is an early sign of imbalance."),Te("cell mean","Average voltage across all of the pack’s cells."),Te("pack volt","Pack terminal voltage."),Te("rep temp","Representative pack temperature reported by the BMS."),Te("cell max|cell min","Hottest / coldest individual cell temperature in the pack."),Te("cell temperatures","Per-cell temperature sensors inside the pack."),Te("cell voltages","Per-cell voltage, each shown with its deviation from the pack mean."),Te("mos max|mosfet temperatures|mosfet temps|mosfet","Power-MOSFET temperature — the BMS switching transistors."),Te("board","BMS circuit-board temperature."),Te("shunt","Current-shunt temperature — the precision resistor the BMS measures pack current across."),Te("ptc heater temperatures|ptc heater temps|ptc","PTC heater temperature — keeps the cells warm enough to charge safely in the cold."),Te("cycles","Equivalent full charge/discharge cycles the pack has completed — a measure of battery age."),Te("lifetime throughput","Total energy ever charged into and discharged out of the pack."),Te("capacity","Energy the battery can store, in kWh."),Te("balancing|cells balancing","The BMS is equalizing cell voltages — routine housekeeping, no action needed."),Te("hottest pack","The warmest pack across the fleet right now."),Te("vitals","The pack’s key live readings at a glance."),Te("pv|pv in|pv total|photovoltaic","Photovoltaic — solar-panel power."),Te("pv high mppt|pv low mppt","Power from one of the DPU’s two solar strings (high- or low-voltage MPPT input)."),Te("ac out|ac output","AC power flowing out of the inverter to your loads."),Te("ac in","AC power flowing into the inverter — grid or generator charging."),Te("ac out freq / v","Inverter AC output frequency (Hz) and voltage."),Te("total in / out","Total power into and out of the DPU across every input and output."),Te("battery v / a","Internal battery-bus voltage and current."),Te("in|out","Power flowing in to / out of the device."),Te("input|output","Power flowing into (charging) or out of (discharging) the pack."),Te("panel load","Total power the SHP2’s circuits are drawing right now."),Te("live contribution|live draw","Power this device is feeding/drawing right now."),Te("voltage|current","Live electrical voltage / current at this input."),Te("v × a","Voltage × current — instantaneous power, shown as a cross-check on the reported watts."),Te("string ω","Effective resistance (volts ÷ amps) at the MPPT string’s operating point."),Te("mppt|mppt temp|mppt hv|mppt lv|hv mppt|lv mppt","MPPT — the solar charge controller (Maximum Power Point Tracker). Each DPU has two: a high-voltage and a low-voltage string input."),Te("hv channels|lv channels","High-/low-voltage MPPT solar string inputs — one of each per DPU."),Te("ghi","Global Horizontal Irradiance — total sunlight energy on a flat surface (W/m²); the raw “how sunny” number the forecast is built from."),Te("producing now","Solar power being generated right now."),Te("peak today","The highest solar power reached so far today."),Te("coefficient|peak response|response coefficient","Learned response coefficient — watts of PV produced per W/m² of sunlight. Captures panel size, orientation, shading and inverter clipping."),Te("strongest hour","The hour of day your arrays convert sunlight to power most efficiently — reveals their orientation."),Te("observed peak pv","The highest PV output actually recorded at this hour-of-day."),Te("soiling","Dust/pollen on the panels cutting output. Detected by comparing clear-sky production to the cleanest day on record."),Te("output drop","How far clear-sky solar output has fallen below the clean-panel baseline — the soiling indicator."),Te("backup|backup pool","SHP2 backup pool — the combined battery the Smart Home Panel draws on."),Te("backup %","Backup-pool state of charge, trended over the last hour."),Te("reserve floor|backup reserve|reserve","Reserve floor — the state of charge held back for backup. Loads begin shedding below it."),Te("solar reserve","Target state of charge to keep in reserve specifically when running on solar."),Te("mid-priority floor","The SoC at which mid-priority circuits are cut to protect the battery."),Te("charge power","Power currently flowing into the battery."),Te("charge time","Estimated time to fully charge the battery."),Te("rated power","The device’s rated maximum power output."),Te("ems bat temp","Battery temperature as reported by the SHP2’s energy-management system."),Te("hw link","Hardware (wired) link status between the SHP2 and this DPU."),Te("load-shed strategy","The SHP2’s automatic plan for dropping circuits as the battery depletes."),Te("smart backup mode","The SHP2’s backup-behaviour mode setting."),Te("charge schedule","The SHP2’s time-of-use scheduled charging windows."),Te("error code|direct errors|shp2 errors","Device-reported error code — 0 means no fault."),Te("charging power","Power the EV charger is drawing, over the last 24 hours."),Te("sessions today","Charging sessions detected today — a sustained draw above 1 kW."),Te("host dpu|dpu battery","The Delta Pro Ultra the EV charger is wired to — that DPU’s AC output equals the charging draw."),Te("direct telemetry|direct evse telemetry","Raw data straight from the device over MQTT, rather than inferred."),Te("solar next 24 h|solar next 24h","Projected solar production, from the cloud forecast run through your learned array model."),Te("forecast load|forecast load 24 h|typical solar / day","Projected household load from the typical-day consumption curve."),Te("forecast pv","Projected PV output for this hour."),Te("projected low soc","The lowest the battery is forecast to reach over the next 24 hours."),Te("cloud cover","Forecast cloud cover — what derates the solar prediction each hour."),Te("outlook","At-a-glance battery comfort vs the reserve floor: Comfortable, Watch or Tight."),Te("history depth","Days of recorded data behind the forecast and learned models — they sharpen as it grows."),Te("confidence","How trustworthy the learned model is, based on how many samples it has."),Te("z-score|peer z-score","Modified z-score — how many robust deviations a reading sits from normal. Higher = more anomalous; ≥ 3.5 flags, ≥ 5 warns."),Te("fit quality|fit r²","R-squared — how well the trend line fits the data, 0–1. Higher means a more trustworthy projection."),Te("samples|regression samples","How many data points the estimate is built from — more points, more reliable."),Te("sibling median","The median reading across the pack’s four siblings — the “normal” this pack is compared against."),Te("this pack","This pack’s current reading."),Te("deviation","How far this reading sits from the expected/normal value."),Te("baseline window","The span of history and number of samples behind the self-baseline."),Te("decline rate|rise rate","How fast the value is changing, per unit time."),Te("end-of-life|eol|projected eol|reaches 80%","End of life — the 80%-SoH point where a pack has lost a fifth of its original capacity; the conventional LFP replacement mark."),Te("fade rate|fade / yr|avg fade rate","How fast measured capacity (State of Health) is falling — SoH percentage points lost per year."),Te("service left|years left|years to eol","Projected years of service remaining before the pack reaches the 80% end-of-life threshold."),Te("eol threshold","The State of Health at which a pack counts as end-of-life — conventionally 80% for LFP cells."),Te("packs projecting","How many packs have a firm enough SoH trend to project an end-of-life date."),Te("soonest eol","The pack across the fleet projected to reach end-of-life first."),Te("cycles at eol","Projected equivalent full-cycle count by the time the pack reaches end-of-life."),Te("data span","Days of recorded history the projection is regressed over."),Te("projection notes","Plain-language end-of-life verdict for each pack with a firm fade trend."),Te("trend","Whether a pack has a projected fade trend, is stable, is still learning, or has no data yet."),Te("critical","Critical — an immediate problem that needs attention now."),Te("warnings|warning","Warning — something to investigate soon."),Te("informational|info","Informational — noted for awareness, not urgent."),Te("anomalies","Things unusual right now — flagged by peer comparison and self-baseline."),Te("forecasts","Where things are heading — runtime, degradation and day-ahead projections."),Te("actionable","Critical + warning items that may need attention."),Te("recently cleared","Alerts that were raised and have since resolved, with how long each lasted."),Te("today","Energy totals since local midnight."),Te("solar produced","Total solar energy harvested today."),Te("batteries","Net battery energy today — negative means net charged, positive means net discharged.");let ze=class extends de{constructor(){super(...arguments),this.tone="neutral"}render(){return V`<slot></slot>`}};ze.styles=[we,n`
      :host {
        display: inline-flex;
        align-items: center;
        font-size: 0.75rem;
    ...
    `],t([ve({reflect:!0})],ze.prototype,"tone",void 0),ze=t([pe("ef-badge")],ze);let Oe=class extends de{constructor(){super(...arguments),this.label="",this.value="",this.unit=""}render(){return V`
    ...
    `],t([ve()],Oe.prototype,"label",void 0),t([ve()],Oe.prototype,"value",void 0),t([ve()],Oe.prototype,"unit",void 0),Oe=t([pe("ef-tile")],Oe);let Re=class extends de{constructor(){super(...arguments),this.title=""}render(){return V`
      <header>
...
</ha-card>`;const s=Object.values(e.devices),r=function(e){return[...e].sort(_e)}(s),a=r.find(e=>"shp2"===e.projection?.kind),i=r.filter(e=>e.productName.toLowerCase().includes("delta pro ultra")),o=r.filter(e=>e!==a&&!i.includes(e)).sort((e,t)=>Number(t.online)-Number(e.online)),n=e.alerts??[],l=xe(n),c=s.filter(e=>e.online).length,d=(e=>{if(!e)return"never";const t=Math.floor((Date.now()-e)/1e3);return t<60?`${t}s ago`:t<3600?`${Math.floor(t/60)}m ago`:`${Math.floor(t/3600)}h ago`})(e.generatedAt??null);return V`<ha-card>
<div class="header"><div>
<div class="title">${t}</div>
...
${l?q`<text x=${e+12} y=${t+r/2+8} fill=${n} font-size=${c?26:22}>${l}</text>`:K}</g>`,H=this.socColor(h),j=d>5?`▼ ${Se(d)} discharging`:d<-5?`▲ ${Se(-d)} charging`:"idle";return V`<ef-section .title=${"Energy flow"}>
<ef-badge slot="header" tone=${u?"warn":"ok"}>${u?"off-grid":"grid-tied"}</ef-badge>
<div class="flow-wrap"><svg viewBox="0 0 ${720} ${260}" preserveAspectRatio="xMidYMid meet">${T([f+g,v+m/2],[x,k+S/2],r,"#d97706")}${T([$+y,b+w/2],[x,k+S/2],o,"#586474")}${T([x+_,k+S/2],[A,P+E/2],Math.max(p,n),"#15803d")}${M(f,v,g,m,"Solar","42 panels",Se(r),"#d97706","☀",!1)}${M($,b,y,w,"Grid",u?"islanded":"imported",Se(o),u?"#586474":"#0e7490","⌁",!1)}${M(x,k,_,S,`Batteries (${t.length} DPU)`,j,Ae(h,1),H,null,!0)}${M(A,P,C,E,"Loads",`${s?.projection.circuits.filter(e=>(e.watts??0)>1).length??0} circuits`,Se(p),"#15803d","⌂",!1)}</svg></div>
...
<div><div class="row-flex"><span class="soc-big">${Ae(s)}</span><span class="muted-sm">${Me("reserve")} ${Ae(r)}</span></div>
<div class="bar bar-mt6"><span style="width:${s??0}%;background:${this.socColor(s)};"></span></div>
</div>
<div class="kv-grid">
<span class="k">${Me("panel load")}</span><span class="v">${Se(a)}</span>
<span class="k">${Me("charge time")}</span><span class="v">${Ce(t.backupChargeTimeMin)}</span>
...
<span class="k">${Me("charge power")}</span><span class="v">${Se(t.chargeWattPower)}</span>
</div>
${o.length?V`<div><div class="section-header">Top circuits · ${i} active</div>
<div class="kv-grid">${o.map(e=>V`<span class="k" title=${e.name}>${e.name}</span><span class="v">${Se(e.watts)}</span>`)}</div></div>`:K}
...
<div class="row-flex"><span class="pct">${Ae(e.batteryPercentage)}</span><span>slot ${e.slot}</span></div>
<div class="bar bar-mt4"><span style="width:${e.batteryPercentage??0}%;background:${this.socColor(e.batteryPercentage)};"></span></div>
<div class="bar-mt4">${Se(null!=t?-t:null)} · ${Pe(e.emsBatTemp)}
        </div>
      </div>
...
<div><div class="row-flex"><span class="soc-big">${Ae(i,o)}</span><span class="muted-sm">${null!=n?`${Ce(n)} remain`:"—"}</span></div>
<div class="bar bar-mt6"><span style="width:${i??0}%;background:${this.socColor(i)};"></span></div>
</div>
<div class="kv-grid">
<span class="k">${Me("pv")}</span><span class="v">${Se(s?.pvTotalWatts)}</span>
<span class="k">${Me("ac out")}</span><span class="v">${Se(s?.acOutWatts)}</span>
<span class="k">${Me("ac in")}</span><span class="v">${Se(s?.acInWatts)}</span>
<span class="k">${Me("mppt")} temp</span><span class="v">HV ${Pe(s?.mpptHvTemp)} · LV ${Pe(s?.mpptLvTemp)}</span>
<span class="k">${Me("total in / out")}</span><span class="v">${Se(s?.totalInWatts)} · ${Se(s?.totalOutWatts)}</span>
</div>
<div><div class="section-header">${l} battery packs${r?"":" (needs WiFi)"}</div>
<div class="pack-row">${Array.from({length:l},(e,t)=>{const r=s?.packs[t];return r?V`<div class="pack"><div class="n">Pack ${r.num}</div><div class="v">${Ae(r.soc)}</div>
<div class="bar bar-mt2"><span style="width:${r.soc??0}%;background:${this.socColor(r.soc)};"></span></div>
<div class="n" class="bar-mt2">${Pe(r.temp)}</div></div>`:V`<div class="pack empty"><div class="n">Pack ${t+1}</div><div class="v">—</div><div class="n">no data</div></div>`})}</div></div>
...
${null!=r?V`<div><div class="row-flex"><span class="soc-big soc-sm">${Ae(r)}</span><span class="muted-sm">${Pe(a)}</span></div>
<div class="bar bar-mt4"><span style="width:${r}%;background:${this.socColor(r)};"></span></div></div>`:K}
${t?V`<div class="kv-grid"><span class="k">${Me("in")}</span><span class="v">${Se(i)}</span>
<span class="k">${Me("out")}</span><span class="v">${Se(o)}</span>
${null!=n?V`<span class="k">${Me("pv")}</span><span class="v">${Se(n)}</span>`:K}</div>`:K}
