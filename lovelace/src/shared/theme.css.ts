...
/**
 * Single source of truth for visual tokens shared across every card.
 * Cards include `themeCss` in their `static styles = [themeCss, css\`…\`]`
 * to inherit both the CSS-custom-property palette and the glossary
 * tooltip styles, since both live on the host.
 *
 * Tokens map to HA's own custom properties when available so dark/light
 * themes Just Work, with fallbacks for the dev harness.
 */
